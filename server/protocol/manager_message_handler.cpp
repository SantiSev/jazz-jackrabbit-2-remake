...
void MatchesManagerMessageHandler::handle_recv_command(const CommandDTO& command) {
    ...
    int id_match = matches_manager.get_client_by_id(id_client)->get_current_match_id();
...
void MatchesManagerMessageHandler::handle_recv_finish_match() {
    matches_manager.check_matches_status();
}
