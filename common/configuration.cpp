...
void Configuration::load_yaml_info(const std::shared_ptr<YAML::Node>& config) {
    ...
    player_spawn_cd = player["spawn_cd"].as<int>();
    player_invincivility_cd = player["invincivility_cd"].as<int>();
    player_intoxication_cd = player["intoxication_cd"].as<int>();
    auto bullet = (*config)["bullet"];
    bullet_base_dmg = bullet["damage"].as<int>();
    bullet_base_speed = bullet["speed"].as<int>();
    bullet_base_ammo = bullet["ammo"].as<int>();
    bullet_base_max_ammo = bullet["max_ammo"].as<int>();
    bullet_base_shoot_rate = bullet["shoot_rate"].as<int>();
}
