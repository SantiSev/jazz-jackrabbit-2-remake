...
La comunicación entre MatchesManager y sus Match es mayormente entre colas internas, manejadas por dos manejadores de mensajes: ManagerMessageHandler quien recibe todo del cliente y realiza las operaciones y respuestas necesarias al cliente, y las acciones y cheats del jugador las comunica mediante cola interna a su respectiva Match; y el MatchMessageHandler se encarga analogamente a cada jugador realizar las acciones o operaciones indicadas por el mensaje del cliente.
La desconexión del cliente resulta tanto en el "lobby", que primero le avisa a su partida en la que se encontraba y lo elimina de la partida, y luego elimina la conexión con el protocolo del cliente del servidor.
