...
if [ "$G_TESTING_MODEL" != "no-server" ]; then
    assert_program_is_executable "$G_EXECUTABLES_FOLDER/server_test_two"
fi

assert_program_is_executable "$G_EXECUTABLES_FOLDER/client_test_two"
...
spawn_server () {
    ...
    if [ "$G_VALGRIND_MODE" = "valgrind" ]; then
        server_cmd=$(cmd_to_run_with_valgrind "./server_test_two" "__server_valgrind__")
    else
        server_cmd="./server_test_two"
    fi
...
spawn_tiburoncin_and_client () {
    ...
    if [ "$G_VALGRIND_MODE" = "valgrind" ]; then
        client_cmd=$(cmd_to_run_with_valgrind "./client_test_two" "__client_${N}_valgrind__")
    else
        client_cmd="./client_test_two"
    fi
...
for case_folder in $G_CASES_FOLDER/*; do
    ...
    if [ "$G_TESTING_MODEL" != 'no-server' ]; then
        create_link_to_executable "$G_EXECUTABLES_FOLDER/server_test_two"
    fi
    create_link_to_executable "$G_EXECUTABLES_FOLDER/client_test_two"
